...
object Parser {
  ...
  def multilineComment[_: P] = P("/" ~ "*".rep(1) ~ CharsWhile(c => c != '/' && c != '*') ~ "*/")
  ...
  object StatementParsers {
    ...
    def statement[_: P]: P[Statement] =
      (ss ~ (ifStatement | letStatement.log | whileStatement | doStatement | returnStatement) ~ ss)
  ...
  private def parser[_: P] = P(
    (ss ~ (StructureParsers.classDeclaration | StatementParsers.statement | ExpressionParsers.expression.log))
  )
